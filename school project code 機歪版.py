...
# 設定遊戲標題
pygame.display.set_caption('Flappy Bird 機歪版')
...
# 載入遊戲圖片
button_img = pygame.image.load('D:\機歪版/restart.png')
background = pygame.image.load('D:\機歪版/background.png')
ground_img = pygame.image.load('D:\機歪版/ground.png')
chicken = pygame.image.load('D:\機歪版/chicken4.png')
...
class Bird(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        for num in range(1, 4):  # 载入鸟的图片
            img = pygame.image.load(f"D:\機歪版/bird{num}.png")
    ...
    def update(self):
        ...
        if game_over == False:
            ...
            if keys[K_SPACE] and self.clicked == False:
                self.clicked = True
                self.vel = random.randint(-10,-5) # 设置向上跳跃的速度
            if not keys[K_SPACE]:
                self.clicked = False
...
class Pipe(pygame.sprite.Sprite):
	...
	def __init__(self, x, y, position):
		...
		self.image = pygame.image.load("D:\機歪版/pipe.png")	 # 載入管道的圖片
...
class Button2():
    def __init__(self, x, y, image):
        ...
        self.rect = self.image.get_rect()  # 获取图片的矩形区域
        self.rect.topleft = (x, y)  # 设置按钮的左上角位置
...
# 創建重新開始按鈕物件
button = Button2(90, screen_height // 2 - 100, button_img)
